...
import { PediatricAgeInput } from '../inputs/PediatricAgeInput.jsx';
import {
...
function FieldControl({ field, value, values, onChange, onPatch }) {
  if (field.type === 'pediatricAge') {
    return (
      <PediatricAgeInput
        ...
        onChange={(id, nextValue) => onPatch({ [id]: nextValue })}
      />
    );
  }
      ...
      <div className="tool-fields">
        {protocol.assessment.fields.map((field) => (
          <label className={field.type === 'checkbox' && values[field.id] ? 'tool-field is-checked' : 'tool-field'} key={field.id}>
            ...
            <FieldControl field={field} value={values[field.id]} values={values} onChange={(value) => updateValue(field, value)} onPatch={patchValues} />
          </label>
        ))}
      </div>
